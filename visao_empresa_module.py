...
def country_maps(df3):
    ...
    df_aux = df3.loc[:, cols].groupby(['City', 'Road_traffic_density']).median().reset_index()
...
def Traffic_Order_City(df3):
    #Gráfico de Bolhas
    df_aux = (df3.loc[:, ['ID', 'City','Road_traffic_density']]
...
def Traffic_Order_Share(df3):
    df_aux = (df3.loc[:, ['ID', 'Road_traffic_density']]
...
def clean_code(df3):
    """ Esta função realiza a limpeza no dataframe
